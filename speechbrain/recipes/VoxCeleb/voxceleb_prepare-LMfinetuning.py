    ...
    for data_folder in data_folders:
        ...
        if split_speaker:
            ...
            for f in glob.glob(path, recursive=True):
                spk_id = f.split("/voxceleb2_2000/")[1].split("/")[1]
...
def prepare_csv(seg_dur, wav_lst, csv_file, random_segment=False, amp_th=0):
    ...
    csv_output = [["ID", "duration", "wav", "start", "stop", "spk_id","data_set"]]
    ...
    with open(csv_file + "path", mode="w") as csv_f:
        csv_writer = csv.writer(
            csv_f, delimiter=",", quotechar='"', quoting=csv.QUOTE_MINIMAL
        )
        for line in wavpath:
