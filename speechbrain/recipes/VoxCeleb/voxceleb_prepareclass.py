...
def prepare_voxceleb(
    ...
    if "train" in splits:
        prepare_csv(
            seg_dur, data_folder, save_csv_train, random_segment, amp_th
        )
...
def prepare_csv(seg_dur, wav_lst, csv_file, random_segment=False, amp_th=0):
    ...
    for wav_file in tqdm(wav_lst, dynamic_ncols=True):
        ...
        try:
            [datset,spk_id, sess_id, utt_id] = wav_file.split("/")[-4:]
        except ValueError:
            logger.info(f"Malformed path: {wav_file}")
            continue
        audio_id = my_sep.join([datset,spk_id, sess_id, utt_id.split(".")[0]])
