...
def prepare_voxceleb(
    ...
    save_csv_movise = os.path.join(save_folder,MOVIRS)
 ...
 #       if not os.path.exists(os.path.join(data_folder, "wav", "id10270")):
  #          logger.info(f"Extracting {source}/{TEST_WAV} to {data_folder}")
   #         shutil.unpack_archive(os.path.join(source, TEST_WAV), data_folder)
    ...
    wav_lst_movies = _get_utt_movies_lists(movies_folder)
    #print(wav_lst_dev[1])
    # with open('/Share/dyb/10%vox1+2.txt', mode="w") as csv_f:
    #     csv_writer = csv.writer( csv_f)
    #     for line in wav_lst_dev:
    #         csv_writer.writerow(line)
    #with open('/Share/dyb/10%vox1.txt', 'w') as f:  # 打开test.txt   如果文件不存在，创建该文件。
       # for i in wav_lst_dev:
          #  print(i)
           # f.writelines(i.split("voxceleb1+2/")[1]+"\n")
    ...
    prepare_csv(seg_dur,wav_lst_movies,save_csv_movise,random_segment,amp_th)
...
def _get_utt_split_lists(
    ...
    for data_folder in data_folders:
        ...
        test_spks = [snt.split("/")[1] for snt in test_lst]
...
def _get_utt_movies_lists(data_folders):
    """
    Tot. number of speakers vox1= 1211.
    Tot. number of speakers vox2= 5994.
    Splits the audio file list into train and dev.
    ...
    """
    movies_lst = []
    print(data_folders,"111")
    print("Getting file list...")
    path = os.path.join(data_folders, "**","*.wav")
      # avoid test speakers for train and dev splits
    audio_files_list = []
    for f in glob.glob(path, recursive=True):
          audio_files_list.append(f)
    random.shuffle(audio_files_list)
    movies_snts = audio_files_list
    movies_lst.extend(movies_snts)
...
def prepare_csv(seg_dur, wav_lst, csv_file, random_segment=False, amp_th=0):
    ...
    csv_output = [["ID", "duration", "wav", "start", "stop", "spk_id", "dataset"]]
    ...
    for wav_file in tqdm(wav_lst, dynamic_ncols=True):
        ...
        try:
            [dataset,spk_id, sess_id, utt_id] = wav_file.split("/")[-4:]
        except ValueError:
            logger.info(f"Malformed path: {wav_file}")
            continue
        audio_id = my_sep.join([dataset,spk_id, sess_id, utt_id.split(".")[0]])
...
def prepare_csv_enrol_test(data_folders, save_folder, verification_pairs_file):
    ...
    for a in data_folders :
        ...
        for line in open(test_lst_file):
            ...
            t_id = line.split(" ")[1].rstrip().split(".")[0].strip()
        ...
        for id in enrol_ids:
            ...
            start_sample = 0
            stop_sample = signal.shape[0]
            [spk_id, utt_id] = wav.split("/")[-1].rsplit("_",1)[-2:]
