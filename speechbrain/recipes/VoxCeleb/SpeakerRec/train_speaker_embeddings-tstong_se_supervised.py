...
from speechbrain.nnet.losses import SupConLoss
import numpy as np


class SpeakerBrain(sb.corese3.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            for count, augment in enumerate(self.hparams.augment_pipeline):
                ...
                if self.hparams.concat_augment:
                    ...
                    wavs = wavs_aug
                    wavs_aug_tot[0] = wavs
            # wavs_aug_tot = torch.tensor(np.array(wavs_aug_tot))
            wavs_T = wavs
            # print(wavs_T.shape)
            wavs_S = wavs_aug_tot[0]
        ...
        feats_T = self.modules.compute_features(wavs1)
        feats_T = self.modules.mean_var_norm(feats_T,lensC)
        feats_S = self.modules2.compute_features(wavs1)
        feats_S = self.modules2.mean_var_norm(feats_S, lensC)
        feats_C = self.modules2.compute_features(wavs1)
        feats_C = self.modules2.mean_var_norm(feats_C, lensC)
        # Embeddings + speaker classifier
        #print(feats_T,"feats_T")
        #print(feats_S,"feats_S")
        embeddings_T,y_1= self.modules.embedding_model(feats_T)
        outputs_T = self.modules.classifier(embeddings_T)
        embeddings_S,y_2= self.modules2.embedding_model(feats_S)
        ...
        return outputs_T,outputs_S,outputs_C ,len_T,len_S,lensC,embeddings_T,embeddings_S,embeddings_C,y_3,y_1
    ...
    def compute_objectives2(self, predictions,lens, batch, stage):
        ...
        return loss
    def compute_objectives3(self, predictions, batch, stage):
        """Computes the loss using speaker-id as label.
        """
        # predictions, lens = predictions
        uttid = batch.id
        ...
        if stage == sb.Stage.TRAIN:
            spkid = spkid
            spkids=spkid.shape[0]
            f1, f2 = torch.split(predictions, [spkids, spkids], dim=0)
            predictions = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)

            # print(predictions.shape
            predictions = predictions.squeeze(dim=3)
        # predictions = self.MSE(predictions_T,predictions_S)#16*1*192   [bsz, n_views, f_dim]
        # loss = self.hparams.compute_cost3(predictions, spkid)
        criterion = SupConLoss(temperature=0.1)
        loss = criterion(predictions, spkid)
        if stage == sb.Stage.TRAIN and hasattr(
                self.hparams.lr_annealing, "on_batch_end"
        ):
            self.hparams.lr_annealing.on_batch_end(self.optimizer)
        ...
        return loss
    def compute_objectives4(self, predictions, batch, stage):
        """Computes the loss using speaker-id as label.
        """
        # predictions, lens = predictions
        uttid = batch.id
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.VALID:
            ...
            sb.nnet.schedulers.update_learning_rate(self.optimizer2, new_lr2)
            ...
            self.hparams.train_logger2.log_stats(
                stats_meta={"epoch": epoch, "lr": old_lr2},
                train_stats=self.train_stats,
                valid_stats=stage_stats,
            )
...
if __name__ == "__main__":
    ...
    sb.corese3.create_experiment_directory(
