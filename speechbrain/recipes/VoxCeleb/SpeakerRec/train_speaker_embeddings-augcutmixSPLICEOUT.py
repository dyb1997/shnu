...
class SpeakerBrain(sb.core1.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        r = np.random.rand(1)
        if r < 0.5:
            wavs, y_a= self.cutmix(wavs, spkid, 0.5)
        else:
            wavs = wavs
            y_a = spkid
        # print(wavs.shape)
        ...
        return outputs, lens,y_a

    def compute_objectives(self, predictions, batch, stage):
        """Computes the loss using speaker-id as label.
        """
        predictions, lens,y_a = predictions
        uttid = batch.id
        # spkid, _ = batch.spk_id_encoded
        ...
        loss = self.hparams.compute_cost(predictions, y_a, lens)
    ...
    def cutmix(self,input,target,beta,use_cuda=True):
        ...
        cx1 = np.random.randint(3200)
        cx2 = np.random.randint(3200)
        input = torch.cat([input[:,:bbx1],input[:,bbx1+cx1:],input[:,:bbx2],input[:,bbx2+cx2:]],-1)
        # adjust lambda to exactly match pixel ratio
        # lam = 1 - ((bbx2 - bbx1)  / input.size()[-1] )
        return input,target_a
