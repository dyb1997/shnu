...
class SpeakerBrain(sb.core1.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        #     arget_a, target_b, target_reweighted, feats, total_unchanged_portion = self.modules.patchup(feats,spkid,lam=None)
        # else:
        #     feats = feats
        #     arget_a  = spkid
        #     target_b, target_reweighted, total_unchanged_portion = None, None, None
        # Embeddings + speaker classifier
        # embeddings,arget_a,target_b,target_reweighted,total_unchanged_portion = self.modules.embedding_model(feats,spkid,stage)
        embeddings,arget_a,target_b,target_reweighted,total_unchanged_portion = self.modules.embedding_model(feats)
        outputs = self.modules.classifier(embeddings)

        return outputs, lens,arget_a,target_b,target_reweighted,total_unchanged_portion

    def compute_objectives(self, predictions, batch, stage):
        """Computes the loss using speaker-id as label.
        """
        # predictions, lens,y_a,y_b,lam = predictions
        outputs, lens, arget_a, target_b, target_reweighted, total_unchanged_portion=predictions
        uttid = batch.id
        # spkid, _ = batch.spk_id_encoded
        ...
        #     spkid = torch.cat([spkid] * self.n_augment, dim=0)
        if target_b != None:
            loss = self.hparams.compute_cost(outputs, arget_a, lens) * \
                   total_unchanged_portion + self.hparams.compute_cost(outputs, target_b, lens) * (1. - total_unchanged_portion) + \
        ...
        if stage != sb.Stage.TRAIN:
            self.error_metrics.append(uttid, outputs, arget_a, lens)
            self.error_metrics.append(uttid, outputs, arget_a, lens)
