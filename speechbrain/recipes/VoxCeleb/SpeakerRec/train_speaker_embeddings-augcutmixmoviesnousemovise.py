...
class SpeakerBrain(sb.core1cutmix.Brain):
    ...
    def compute_forward(self, batch, wavs2, spkids, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            wavs = torch.cat([wavs, wavs2], dim=0)
            spkid = torch.cat([spkid, spkids], dim=0)
        ...
        # print(lens.shape)
        r = np.random.rand(1)
        if r < 0.5 and stage==sb.Stage.TRAIN:
            lena=len(wavs)/2
            # wavs=wavs[0:lena,:]
...
def dataio_prep(hparams):
    ...
    movise_folder = hparams["movise_folder"]
    # 1. Declarations:
    ...
    movise_data = sb.dataio.dataset.DynamicItemDataset.from_csv(
        csv_path=hparams["movise_annotation"],
        replacements={"data_root": movise_folder}
    )
    ...
    return train_data, valid_data, movise_data, label_encoder
...
if __name__ == "__main__":
    ...
    from voxceleb_prepareFFSVC import prepare_voxceleb  # noqa
    ...
    run_on_main(
        ...
        kwargs={
            ...
            "movies_folder": hparams["movise_folder"],
    ...
    train_data, valid_data, movise_data,label_encoder = dataio_prep(hparams)

    # Create experiment directory
    sb.core1cutmix.create_experiment_directory(
    ...
    speaker_brain.fit(
        ...
        train_data,
        valid_data,
        movise_csv=hparams["movise_annotation"],
