...
class SpeakerBrain(sb.coreBYOLTSMFA.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            ...
            embeddings_T_sup,b_T1 = self.modules.embedding_model(feats_T_sup)#干净老师FC层
            # outputs_T_sup = self.modules.classifier(embeddings_T_sup)
            embeddings_S_sup,pre,b_S1= self.modules2.embedding_model(feats_S_sup)#加噪学生FC+PRE+mfa
            # outputs_S_sup = self.modules2.classifier(pre)
            embeddings_T_sup2 ,b_T2= self.modules.embedding_model(feats_S_sup)#加噪老师FC
            # outputs_T_sup2 = self.modules.classifier(embeddings_T_sup2)
            embeddings_S_sup2, pre2,b_S2 = self.modules2.embedding_model(feats_T_sup)#干净学生FC+PRE+mfa
            ...
            embeddings_T,b_T= self.modules.embedding_model(feats_T)
            outputs_T = self.modules.classifier(embeddings_T)
            embeddings_S,pre3,b_S = self.modules2.embedding_model(feats_S)
            outputs_S = self.modules2.classifier(embeddings_S)
            return outputs_T,outputs_S,len_T,len_S,embeddings_T_sup,pre,embeddings_T_sup2,pre2,embeddings_T,embeddings_S,b_T,b_S
            # outputs_S = self.modules2.classifier(embeddings_S)
        else:
            feats_T = self.modules.compute_features(wavs_T)
            feats_T = self.modules.mean_var_norm(feats_T, len_T)
        ...
        # Embeddings + speaker classifier
            embeddings_T,b_T = self.modules.embedding_model(feats_T)
            outputs_T = self.modules.classifier(embeddings_T)
            embeddings_S ,pre,b_S= self.modules2.embedding_model(feats_S)
            outputs_S = self.modules2.classifier(embeddings_S)
...
if __name__ == "__main__":
    ...
    sb.coreBYOLTSMFA.create_experiment_directory(
