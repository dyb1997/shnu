...
class SpeakerBrain(sb.coreseunsup.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        for a  in dataset :
            if a == "voxceleb2":
                ...
                i = i + 1
                continue
        wavs1 = (torch.stack(wavs1, dim=1)).T
        lenss=torch.stack(lens1,0)
        # print(lens1.size())
        if stage == sb.Stage.TRAIN:
            ...
            for count, augment in enumerate(self.hparams.augment_pipeline):
                # # Apply augment
                # if count ==1 :
                #  = wavs + np.abs(np.random.normal(0, 0.5 ** 2, wavs.shape))
        ...
        ###########################
        feats_T1 = self.modules.compute_features(wavs1)
        feats_T1 = self.modules.mean_var_norm(feats_T1, lenss)
        feats_S1 = self.modules2.compute_features(wavs1)
        feats_S1 = self.modules2.mean_var_norm(feats_S1, lenss)
        # Embeddings + speaker classifier
        # print(feats_T,"feats_T")
        # print(feats_S,"feats_S")
        embeddings_T, y_T = self.modules.embedding_model(feats_T)
        outputs_T = self.modules.classifier(embeddings_T)
        embeddings_S, y_S = self.modules2.embedding_model(feats_S)
        outputs_S = self.modules2.classifier(embeddings_S)
        ###########################
        embeddings_T1, y_T1 = self.modules.embedding_model(feats_T1)
        outputs_T1 = self.modules.classifier(embeddings_T1)
        embeddings_S1, y_S1 = self.modules2.embedding_model(feats_S1)
        outputs_S1 = self.modules2.classifier(embeddings_S1)
        return outputs_T1, outputs_S1, lenss, embeddings_T, embeddings_S, y_T, y_S
            # outputs_S = self.modules2.classifier(embeddings_S)
    ...
    def compute_objectives(self, predictions, lens, batch, stage):
        ...
        if stage != sb.Stage.TRAIN:
            self.error_metrics.append(uttid, predictions, spkids, lens)
...
def dataio_prep(hparams):
    ...
    @sb.utils.data_pipeline.takes("wav", "start", "stop", "duration","data")
    @sb.utils.data_pipeline.provides("sig")
    def audio_pipeline(wav, start, stop, duration,data):
    ...
    def dataset(wav, start, stop, duration,data):
        return data

    sb.dataio.dataset.add_dynamic_item(datasets, dataset)
    # 3. Fit encoder:
...
if __name__ == "__main__":
    ...
    from voxceleb_prepare import prepare_voxceleb3  # noqa
    ...
    sb.coreseunsup.create_experiment_directory(
