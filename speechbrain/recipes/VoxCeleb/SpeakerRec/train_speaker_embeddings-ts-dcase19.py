...
class SpeakerBrain(sb.core.Brain):
    ...
    def compute_forward(self, batch, stage):
        ...
        if stage == sb.Stage.TRAIN:
            wavsa= wavs.cpu()
            wavs_aug = wavsa + np.abs(np.random.normal(0,0.5 ** 2,wavs.shape))
            wavs_aug =  wavs_aug.to(torch.float32)
            ...
            #     #     wavs_aug_tot[0] = wavs
            wavs_T = wavs
            wavs_S = wavs_aug.cuda()
            #wavs = torch.cat(wavs_aug_tot, dim=0)
            len_S = torch.Tensor(len(wavs_aug)).cuda()
            len_T = lens
    ...
    def on_stage_end(self, stage, stage_loss, epoch=None):
        ...
        if stage == sb.Stage.VALID:
            ...
            self.hparams.train_logger2.log_stats(
                stats_meta2={"epoch": epoch, "lr": old_lr2},
                train_stats=self.train_stats,
                valid_stats=stage_stats,
            )
            ...
            self.checkpointer2.save_and_keep_only(
                meta2={"ErrorRate": stage_stats["ErrorRate"]},
                min_keys=["ErrorRate"],
            )
