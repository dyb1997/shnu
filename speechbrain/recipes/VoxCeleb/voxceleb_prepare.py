...
def prepare_voxceleb2(
    ...
    save_folder,
    verification_pairs_file,
    verification_pairs_file2,
    verification_pairs_file3,
    verification_pairs_file4,
    verification_pairs_file5,
    verification_pairs_file6,
    splits=["train", "dev", "test"],
    split_ratio=[90, 10],
    seg_dur=3.0,
    amp_th=5e-04,
    ...
    # wav_lst_train2, wav_lst_dev2 = _get_utt_split_lists(
    #     data_folder, split_ratio, verification_pairs_file2, split_speaker
    # )
    # wav_lst_train3, wav_lst_dev3 = _get_utt_split_lists(
    #     data_folder, split_ratio, verification_pairs_file3, split_speaker
    # )
    # wav_lst_train4, wav_lst_dev4 = _get_utt_split_lists(
    #     data_folder, split_ratio, verification_pairs_file4, split_speaker
    # )
    # wav_lst_train5, wav_lst_dev5 = _get_utt_split_lists(
    #     data_folder, split_ratio, verification_pairs_file5, split_speaker
    # )
    # # Creating csv file for training data
    if "train" in splits:
        prepare_csv(
    ...
    if "test" in splits:
        ...
        prepare_csv_enrol_test2(
            data_folder, save_folder, verification_pairs_file2
        )
        prepare_csv_enrol_test3(
            data_folder, save_folder, verification_pairs_file3
        )
        prepare_csv_enrol_test4(
            data_folder, save_folder, verification_pairs_file4
        )
        prepare_csv_enrol_test5(
            data_folder, save_folder, verification_pairs_file5
        )
        prepare_csv_enrol_test6(
            data_folder, save_folder, verification_pairs_file6
        )


    # Saving options (useful to skip this phase when already done)
    save_pkl(conf, save_opt)
def prepare_voxceleb3(
    data_folder,
    save_folder,
    verification_pairs_file,
    splits=["train", "dev", "test"],
    ...
    if "dev" in splits:
        prepare_csv_unsup(seg_dur, wav_lst_dev, save_csv_dev, random_segment, amp_th)
...
def _get_utt_split_lists(
    ...
    for data_folder in data_folders:
        ...
        if split_speaker:
            ...
            for f in glob.glob(path, recursive=True):
                try:
                    spk_id = f.split("/wav/")[1].split("/")[1]#voxceleb
                    # spk_id = f.split("/data/")[1].split("/")[1]
                except ValueError:
...
def prepare_csv_unsup(seg_dur, wav_lst, csv_file, random_segment=False, amp_th=0):
    """
    ...
    csv_output = [["ID", "duration", "wav", "start", "stop", "spk_id","data"]]
    ...
    for wav_file in tqdm(wav_lst, dynamic_ncols=True):
        ...
        try:
            [data,spk_id, sess_id, utt_id] = wav_file.split("/")[-4:]
        except ValueError:
            logger.info(f"Malformed path: {wav_file}")
            continue
        audio_id = my_sep.join([data,spk_id, sess_id, utt_id.split(".")[0]])
...
def prepare_csv_enrol_test(data_folders, save_folder, verification_pairs_file):
    ...
    for data_folder in data_folders:
        ...
        for line in open(test_lst_file):
            e_id = line.split(" ")[1].rstrip().split(".wav")[0].strip()
            t_id = line.split(" ")[2].rstrip().split(".wav")[0].strip()
        ...
        enrol_csv = []
        print(verification_pairs_file.split("/")[-1] == "veri_test2.txt")
        for id in enrol_ids:
            if verification_pairs_file.split("/")[-1] == "veri_test2.txt":
                wav = data_folder.split("voxmovies_train")[0] + "voxceleb1/" +id + ".wav"
            else:
        ...
        with open(csv_file, mode="w") as csv_f:
            ...
            for line in csv_output:
                csv_writer.writerow(line)
def prepare_csv_enrol_test3(data_folders, save_folder, verification_pairs_file):
    """
    Creates the csv file for test data (useful for verification)
    ...
    for data_folder in data_folders:
        ...
        with open(csv_file, mode="w") as csv_f:
            ...
            for line in csv_output:
                csv_writer.writerow(line)
def prepare_csv_enrol_test2(data_folders, save_folder, verification_pairs_file):
    """
    Creates the csv file for test data (useful for verification)
    ...
    for data_folder in data_folders:
        ...
        csv_output = csv_output_head + enrol_csv
        csv_file = os.path.join(save_folder, "2"+ENROL_CSV)
        ...
        csv_output = csv_output_head + test_csv
        csv_file = os.path.join(save_folder, "2"+TEST_CSV)
...
def prepare_csv_enrol_test3(data_folders, save_folder, verification_pairs_file):
    ...
    for data_folder in data_folders:
        ...
        csv_output = csv_output_head + enrol_csv
        csv_file = os.path.join(save_folder, "3"+ENROL_CSV)
        ...
        csv_output = csv_output_head + test_csv
        csv_file = os.path.join(save_folder, "3"+TEST_CSV)
        ...
        with open(csv_file, mode="w") as csv_f:
            ...
            for line in csv_output:
                csv_writer.writerow(line)
def prepare_csv_enrol_test4(data_folders, save_folder, verification_pairs_file):
    """
    Creates the csv file for test data (useful for verification)
    ...
    for data_folder in data_folders:
        ...
        csv_output = csv_output_head + enrol_csv
        csv_file = os.path.join(save_folder,"4"+ ENROL_CSV)
        ...
        csv_output = csv_output_head + test_csv
        csv_file = os.path.join(save_folder, "4"+TEST_CSV)
        ...
        with open(csv_file, mode="w") as csv_f:
            ...
            for line in csv_output:
                csv_writer.writerow(line)
def prepare_csv_enrol_test5(data_folders, save_folder, verification_pairs_file):
    """
    Creates the csv file for test data (useful for verification)
    ...
    for data_folder in data_folders:
        ...
        csv_output = csv_output_head + enrol_csv
        csv_file = os.path.join(save_folder, "5"+ENROL_CSV)
        ...
        csv_output = csv_output_head + test_csv
        csv_file = os.path.join(save_folder, "5"+TEST_CSV)
...
def prepare_csv_enrol_test6(data_folders, save_folder, verification_pairs_file):
    """
    Creates the csv file for test data (useful for verification)
    ...
    for data_folder in data_folders:
        ...
        for id in enrol_ids:
            # wav = data_folder.split("voxmovies_train")[0] + "voxceleb1/" +id + ".wav"
            wav = data_folder +"voxceleb1/"+ id +".wav"
            # Reading the signal (to retrieve duration in seconds)
            signal, fs = torchaudio.load(wav)
            signal = signal.squeeze(0)
            audio_duration = signal.shape[0] / SAMPLERATE
        ...
        csv_output = csv_output_head + enrol_csv
        csv_file = os.path.join(save_folder, "6"+ENROL_CSV)
        ...
        csv_output = csv_output_head + test_csv
        csv_file = os.path.join(save_folder, "6"+TEST_CSV)
