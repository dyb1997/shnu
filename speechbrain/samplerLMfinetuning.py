...
class ReproducibleRandomSampler(RandomSampler):
    ...
    def __init__(self, data_source, seed=563375142, epoch=0, **kwargs):
        ...
        self.data.data_set = data_source.data_set
        self.data.id = data_source
        self.seed = int(seed)
        self.epoch = epoch
...
class PrototypicalBatchSampler(Sampler):
    ...
    def __init__(self, data, classes_per_it=8, num_samples=1, sim_per_it=4, **kwargs):
        '''
        ...
        '''
        lab2ind, ind2lab, extras = sb.dataio.encoder.CategoricalEncoder._load_literal(
            '/Share/dyb/speechbrain/recipes/VoxCeleb1/SpeakerRec/results/ecapa_augment_2+1/2021/save/label_encoder.txt')
        self.drop_last = False
        self.data = data.data  # 104
        self.data_id = list(self.data.keys())
        # self.data = load_data_csv("/mnt/data3/dyb/speechbrain/recipes/VoxCeleb/SpeakerRec/results/ecapa_augment2022/1986/save/train.csv")
        self.labels = ind2lab
        # self.classes, self.counts = np.unique(self.labels, return_counts=True)
        self.classes_per_it = classes_per_it
        self.sample_per_class = num_samples
        self.sim_per_it = sim_per_it
        image = torch.load(
            "/Share/dyb/speechbrain/spk_center.pt")  # 在core.py的fit（）里保存的
        self.weight = image
        self.n_data = len(data)
        # self.allspk = list(range(7204))
        self.allspk = list(range(len(self.labels)))
        self.batch_size = self.sample_per_class * self.classes_per_it * self.sim_per_it

    def __iter__(self):
        '''
        yield a batch of indexes
        '''
        # spc = self.sample_per_class  # 每一类的样本数，没个说话人有U句话
        # cpi = self.classes_per_it  # 每个batch的类数，说话人S
        # sim = self.sim_per_it  # 相似的说话人数量I
        # self.sampler = []
        # print(self.allspk)
        while len(self.allspk) > 0:
            if len(self.allspk) >= self.classes_per_it:
                spk1 = random.sample(self.allspk, self.classes_per_it)
            else:
                break
                # spk1 = random.sample(self.allspk, len(self.allspk))
            self.spk1 = list(spk1)
            self.batch = []
            spk = torch.tensor(self.spk1).to(device="cuda")
            spk_sort = torch.index_select(self.weight, 0, spk, out=None)
            for t in spk1:
                if t in self.allspk:
                    self.allspk.remove(t)
            x = 0  # 转换为numpy  #
            for i in spk_sort:
                # spk_t = spk1[x]
                # spk_t = np.array(spk_t)
                # spk_t = spk_t.tolist()
                x = x + 1
                spk_2, index = torch.sort(i, -1, descending=True)  # 第i行排序
                index = index.to(device="cpu")
                index = np.array(index)
                index = index.tolist()
                a = 0
                while a < self.sim_per_it:
                    aaa = self.labels[index[a]]
                    b = index[a]
                    spk4 = []
                    for k, v, in self.data.items():
                        if v.get("spk_id") == aaa:
                            i = self.data_id.index(k)
                            spk4.append(i)
                    # random.sample(spk4.items(), scp                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          )
                    if len(spk4) > 0:
                        num = random.choice(spk4)
                        self.batch.append(num)
                    a = a + 1
                    # print(len(batch))
            # print(batch)
            # print(len(batch))
    ...
    def __len__(self):
        if self.drop_last:
            return len(self.allspk) // self.classes_per_it
        else:
            return (len(self.allspk) + self.classes_per_it - 1) // self.classes_per_it
        # return len(self.labels) //self.sample_per_class
