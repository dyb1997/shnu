...
from speechbrain.lobes.models.ECAPA_TDNN.ECAPA_TDNN import ECAPA
...
def train2(train_loader, model, optimizer, c_epoch, ema_model=None, mask_weak=None, mask_strong=None, adjust_lr=False):
    ...
    Args:
        ...
        mask_weak: slice or list, mask the batch to get only the weak labeled data (used to calculate the loss)弱标签数据
        mask_strong: slice or list, mask the batch to get only the strong labeled data (used to calcultate the loss)强标签数据
    ...
    #bce loss，b这里指的是binary，所以用于二分类问题，在使用nn.BCELoss需要在该层前面加上Sigmoid函数
    consistency_criterion = nn.MSELoss()
    #一致性loss，将两个loss函数放入gpu
    ...
    #enumerate 调用dataloader类的__iter__方法，调用teacher和students模型的输入
    for i, ((batch_input, ema_batch_input), target) in enumerate(train_loader):
        global_step = c_epoch * len(train_loader) + i
        #当前epoch数*样本被分成了多少份+i
        ...
        #T的输入，S的输入，标签都放入GPU
        # Outputs
        strong_pred_ema, weak_pred_ema = ema_model(ema_batch_input)#学生模型的输入，输出一个强标签的后验预测和一个弱标签的后验预测
        ...
        loss = None
        # Weak BCE Loss
        target_weak = target.max(-2)[0]  # Take the max in the time axis取最小的两个target
        #切片或列表，屏蔽批次以仅获取弱标记数据（用于计算损失）
...
if __name__ == '__main__':
    ...
    # Model taken from 2nd of dcase19 challenge: see Delphin-Poulat2019 in the results.模型参数
    n_layers = 7
    ecapa_kwargs = {"input_size": 80,
                    "channels": [1024, 1024, 1024, 1024, 3072],
                    "kernel_sizes": [5, 3, 3, 3, 1],
                    "dilations": [1, 2, 3, 4, 1],"groups": [1, 1, 1, 1, 1],
    ...
    #poolling后会维度降低，为了计算时间戳对齐的
    # pooling_time_ratio = 4  # 2 * 2 poolling后会维度降低，为了计算时间戳对齐的
    #cfg指的是配置文件config.py
    #out_nb_frames_1s = cfg.sample_rate / cfg.hop_size / pooling_time_ratio  #每1秒分多少帧
    #帧长
    ...
    #cfg.classes类列表，cfg.max_frames最大帧//4
    #将标签编码为 numpy 数组，其中 1 对应于类的存在和 0 不存在。多个 1 可以出现在同一行，这是针对多标签问题。
    #many_hot_encoder = ManyHotEncoder(cfg.classes, n_frames=cfg.max_frames // pooling_time_ratio)
    #encod_func = many_hot_encoder.encode_strong_df#对强标签列表（或 pandas Dataframe 或 Serie）进行编码，它们对应于给定的文件名

    # Normalisation per audio or on the full dataset对整个数据集（或者单个语音）做normalisation
    # if cfg.scaler_type == "dataset":
    #     #转换dataload为元组get_transforms
    #     transforms = get_transforms(cfg.max_frames, add_axis=add_axis_conv)
    #     #弱标签，无标签，合成的数据以一个数据集的做标签和元组的转换
    ...
    # Model
    # ##############
    ecapa = ECAPA(**ecapa_kwargs)
    #crnn = CRNN(**crnn_kwargs)
    pytorch_total_params = sum(p.numel() for p in ecapa.parameters() if p.requires_grad)
    ...
    ecapa.apply(weights_init)
    #crnn.apply(weights_init)

    ecapa_ema = ECAPA(**ecapa_kwargs)
    ecapa_ema.apply(weights_init)
    for param in ecapa_ema.parameters():
        param.detach_()
    ...
    optim = torch.optim.Adam(filter(lambda p: p.requires_grad, ecapa.parameters()), **optim_kwargs)
    bce_loss = nn.BCELoss()

    state = {
        'model': {"name": ecapa.__class__.__name__,
                  'args': '',
                  "kwargs": ecapa_kwargs,
                  'state_dict': ecapa.state_dict()},
        # 'model': {"name": ecapa.__class__.__name__,
        #           'args': '',
        #           "kwargs": crnn_kwargs,
        #           'state_dict': crnn.state_dict()},
        'model_ema': {"name": ecapa_ema.__class__.__name__,
                      'args': '',
                      "kwargs": ecapa_kwargs,
                      'state_dict': ecapa_ema.state_dict()},
    ...
    # Train
    # ##############
    results = pd.DataFrame(columns=["loss", "valid_synth_f1", "weak_metric", "global_valid"])#构建列表columns(列标签)
    #在一个epoch中，训练CRNN老师模型，crnn——ema学生模型，将两个模型放入GPU
    for epoch in range(cfg.n_epoch):
        ...
        ecapa.train()
        ecapa_ema.train()
        ecapa,ecapa_ema = to_cuda_if_available(ecapa, ecapa_ema)
        loss_value = train(training_loader, ecapa, optim, epoch,
                           ema_model=ecapa_ema, mask_weak=weak_mask, mask_strong=strong_mask, adjust_lr=cfg.adjust_lr)
        # loss_value = train(training_loader, crnn, optim, epoch,
        #                    ema_model=crnn_ema, mask_weak=weak_mask, mask_strong=strong_mask, adjust_lr=cfg.adjust_lr)
