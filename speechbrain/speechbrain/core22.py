...
from speechbrain.dataio.sampler_finetuning import voxmoviesFTBatchSampler
...
# @sb.utils.checkpoints.register_checkpoint_hooks2
class Brain:
    ...
    * ``evaluate_batch()``
    ...
    def __init__(  # noqa: C901
        ...
        for name, value in self.modules.named_parameters():
              if name != "embedding_model.fc.conv.weight" and name != "embedding_model.fc.conv.bias" and name != "classifier.weight":
                value.requires_grad = False
                # print(value.requires_grad)
        for name, value in self.modules2.named_parameters():
            # if name != "embedding_model.fc.conv.weight" and name != "embedding_model.fc.conv.bias" and name != "classifier.weight":
            value.requires_grad = False
    ...
    def _train_loader_specifics(self, dataset, loader_kwargs):
        ...
        batch_sampler = loader_kwargs.get("sampler", None)
        ...
        if shuffle and not self.distributed_launch:
            ...
            sampler = torch.utils.data.sampler.SequentialSampler(data_source=dataset)
            batch_sampler = voxmoviesFTBatchSampler(sampler)
    ...
    def fit_batch(self, batch):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            outputs_T, outputs_S, len_T, len_S ,embedding_T,embedding_S= self.compute_forward(batch, sb.Stage.TRAIN)
            loss = None
            loss1 = self.compute_objectives(outputs_T, len_T, batch, sb.Stage.TRAIN)
            print("loss1",loss1)
            ...
            if loss is not None:
                ...
                loss = loss1
            loss2 = self.compute_objectives2(outputs_S, len_S, batch, sb.Stage.TRAIN)
            print("loss2",loss2)
            ...
            #loss3 = self.MSE(outputs_T, outputs_S)
            print(loss3)
            meters.update('MSE-loss', loss3.item())
            #loss3 = 0.1*loss3
            if loss is not None:
                loss += loss3
            else:
    ...
    def evaluate_batch(self, batch, stage):
        ...
        * ``compute_objectives()``
        ...
        # loss = loss2 + loss3
        meters = AverageMeterSet()
        outputs_T, outputs_S, len_T, len_S ,embedding_T,embedding_S= self.compute_forward(batch, stage=stage)
        loss1 = self.compute_objectives(outputs_T, len_T, batch, stage=stage)
        loss2 = self.compute_objectives2(outputs_S, len_S, batch, stage=stage)
        loss3 = self.MSE(embedding_T, embedding_S)
        #loss3 = 0.1 * loss3
        loss=loss1+loss2+loss3
