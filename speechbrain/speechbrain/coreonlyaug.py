    ...
    * ``evaluate_batch()``
    ...
    def fit_batch(self, batch):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            outputs, lens1,embeddings2,embeddings3,lens = self.compute_forward(batch, sb.Stage.TRAIN)
            loss1 = self.compute_objectives(outputs, lens1,batch,sb.Stage.TRAIN)
            loss2 = self.MSE(embeddings2,embeddings3)
    ...
    def evaluate_batch(self, batch, stage):
        ...
        * ``compute_objectives()``
        ...
        outputs, lens1,embeddings2,embeddings3,lens = self.compute_forward(batch, stage=stage)
        loss = self.compute_objectives(outputs,lens1, batch, stage=stage)
        return loss.detach().cpu()
