...
class Brain:
    ...
    * ``evaluate_batch()``
    ...
    def compute_forward(self, batch,movise_set, stage):
    ...
    def fit_batch(self, batch,movise_set):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            with torch.cuda.amp.autocast():
                outputs = self.compute_forward(batch,movise_set, Stage.TRAIN )
    ...
    def fit(
        ...
        * ``update_average()``
        ...
        if movise_set is not None and not (
            isinstance(movise_set, DataLoader)
            or isinstance(movise_set, LoopedLoader)
        ):
            movise_set = self.make_dataloader(
                movise_set,
        ...
        for epoch in epoch_counter:
            ...
            with tqdm(
                ...
                for batch in t:
                    self.step += 1
                    loss = self.fit_batch(batch,movise_set)
