    ...
    * ``evaluate_batch()``
    ...
    def fit_batch(self, batch):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            outputs_T1, outputs_S1, lens1, embeddings_T, embeddings_S, y_T, y_S= self.compute_forward(
                batch, sb.Stage.TRAIN)
            loss = None
            loss1 = self.compute_objectives(outputs_T1, lens1, batch, sb.Stage.TRAIN)
            print("loss1", loss1)
            ...
            if loss is not None:
                ...
                loss = loss1
            loss2 = self.compute_objectives2(outputs_S1, lens1, batch, sb.Stage.TRAIN)
            print("loss2", loss2)
            ...
            ##################################
            loss3 = self.MSE(embeddings_T, embeddings_S)
            ...
            loss4 = (consistency_cost+1) * loss4
            # # loss3 = self.MSE(outputs_T, outputs_S)
            print("loss4",loss4)
    ...
    def evaluate_batch(self, batch, stage):
        ...
        * ``compute_objectives()``
        ...
        detached loss
        """
        # outputs_T, outputs_S, len_T, len_S,y_t,y_s,z_t,z_s = self.compute_forward(batch, stage=stage)
        ...
        # loss = loss2 + loss3
        meters = AverageMeterSet()
        outputs_T1, outputs_S1, lens1, embeddings_T, embeddings_S, y_T, y_S= self.compute_forward(batch, stage=stage)
        loss1 = self.compute_objectives(outputs_T1, lens1, batch, stage=stage)
        loss2 = self.compute_objectives2(outputs_S1, lens1, batch, stage=stage)
        #loss3 = self.MSE(embedding_T, embedding_S)
        # consistency_cost = self.max_consistency_cost * self.rampup_value
        # loss3=consistency_cost*loss3
        #loss4 = consistency_costself.MSE(z_t, z_s)
        ...
        * ``update_average()``
        ...
        for epoch in epoch_counter:
            ...
            with tqdm(
                ...
                for batch in t:
                    print("123124124312",len(train_set))
                    self.step += 1
                    ...
                    self.rampup_value2 = self.exp_rampup(self.global_step, self.max_epoch * len(train_set))
