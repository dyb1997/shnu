    ...
    * ``evaluate_batch()``
    ...
    def fit_batch(self, batch):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            accumulation_step = 2
            outputs,len ,emb,y= self.compute_forward(batch, sb.Stage.TRAIN)
            loss = self.compute_objectives3(y, batch, sb.Stage.TRAIN)
            loss = loss / accumulation_step
            loss.backward()
            if self.step % accumulation_step == 0:
                if self.check_gradients(loss):
                    self.optimizer.step()
    ...
    def evaluate_batch(self, batch, stage):
        ...
        * ``compute_objectives()``
        ...
        out, lens, emb, y = self.compute_forward(batch, stage=stage)
        loss = self.compute_objectives(out, lens, batch, stage=stage)
        return loss.detach().cpu()
