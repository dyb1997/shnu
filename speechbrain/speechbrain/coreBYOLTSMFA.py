    ...
    * ``evaluate_batch()``
    ...
    def make_dataloader(
        ...
        **loader_kwargs : dict
        ...
        #     and ckpt_prefix is not None
        #     and (
        #         isinstance(dataloader2, SaveableDataLoader)
        #         or isinstance(dataloader2, LoopedLoader)
        #     )
        # ):
        #     ckpt_key = ckpt_prefix + stage.name
    ...
    def init_optimizers(self):
        ...
        if self.opt_class2 is not None:
            self.optimizer2 = self.opt_class(self.modules2.parameters())

            if self.checkpointer2 is not None:
                self.checkpointer2.add_recoverable("optimizer", self.optimizer)
    ...
    def fit_batch(self, batch):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            outputs_T,outputs_S,len_T,len_S,embeddings_T_sup,pre,embeddings_T_sup2,pre2,embeddings_T,embeddings_S,b_T,b_S= self.compute_forward(batch, sb.Stage.TRAIN)
            loss = None
            #################AAM-SOFTMAX########################
            ...
            loss2 = self.compute_objectives(outputs_S, len_S, batch, sb.Stage.TRAIN)
            print("loss2",loss2)
            ...
            loss6 = self.MSE(embeddings_T, embeddings_S)
            loss6 = consistency_cost * loss6
            # loss3 = self.MSE(outputs_T, outputs_S)
            print("lossFC", loss6)
            meters.update('MSE-loss', loss6.item())
            # loss3 = 0.1*loss3
            if loss is not None:
                loss += loss6
            else:
                loss = loss6
            ###################MSEmfa#####################
            # loss7 = self.MSE(b_T, b_S)
            # loss7 = consistency_cost * loss7
            # # loss3 = self.MSE(outputs_T, outputs_S)
            # print("loss7", loss7)
            # meters.update('MSE-loss1', loss7.item())
            # # loss3 = 0.1*loss3
            # if loss is not None:
            #     loss += loss7
            # else:
            #     loss = loss7
            #############BYOL:t-clean+s-aug###############
            loss3 = self.loss_fn(embeddings_T_sup,pre)
            loss3 = loss3.mean()
            #loss3 = self.MSE(outputs_T, outputs_S)
            print("teacher-student",loss3)
            meters.update('teacher', loss3)
            #loss3 = 0.1*loss3
            # if loss is not None:
            #     loss += loss3
            # else:
            #     loss = loss3
            ##############BYOL:s-clean+t-aug########################
            loss4 = self.loss_fn(embeddings_T_sup2,pre2)
            loss4= loss4.mean()
            # loss3 = self.MSE(outputs_T, outputs_S)
            print("student",loss4)
            meters.update('t2-loss', loss4)
            loss5= loss3+loss4
            # g = (loss1+loss2)/(1+loss5)
            # loss5 = g*loss5
            if loss is not None:
                loss += loss5
            else:
    ...
    def evaluate_batch(self, batch, stage):
        ...
        * ``compute_objectives()``
        ...
        # loss = loss2 + loss3
        meters = AverageMeterSet()
        outputs_T,outputs_S, len_T,len_S,embeddings_T,embeddings_S = self.compute_forward(batch, stage=stage)
        loss1 = self.compute_objectives(outputs_T, len_T, batch, stage=stage)
        loss2 = self.compute_objectives(outputs_S, len_S, batch, stage=stage)
        #loss3 = 0.1 * loss3
        loss=loss1+loss2
