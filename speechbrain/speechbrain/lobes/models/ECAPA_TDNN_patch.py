...
class SEBlock2D(nn.Module):
    ...
    def __init__(self, in_channels, se_channels, out_channels):
        ...
        self.line1=nn.Linear(40, 40 // 16)
        self.relu = torch.nn.ReLU(inplace=True)
        self.line2=nn.Linear(40 // 16,40)
        self.sigmoid = torch.nn.Sigmoid()
...
class ECAPA_TDNNcnnpatch(torch.nn.Module):
    ...
    def __init__(
        ...
        self.patch = PatchUp()
    def forward(self, x,spkid,layer_mix,lengths=None):
        """Returns the embedding vector.
        ...
        if layer_mix==1 and layer_mix <= 2:
            target_a, target_b, target_reweighted, x, portion = self.patch(x, spkid,lam = None)
        else:
            x = x
            target_a  = spkid
            target_b, target_reweighted, portion = None, None, None
        # expectedinput[8, 1, 203, 82]
        for layer in self.blocks2D:
            try:
                x=layer(x,lengths)
                ...
                x=layer(x)
        x = self.b2(x)
        if layer_mix==2 and layer_mix <= 2:
            target_a, target_b, target_reweighted, x, portion = self.patch(x, spkid,lam = None)
        else:
            x = x
            target_a  = target_a
            target_b, target_reweighted, portion = target_b, target_reweighted, portion
        #8, 40, 201, 128
        x=x.permute(0,2,3,1)
        x=x.flatten(2)
        x = x.transpose(1, 2)
        ...
        x = x.transpose(1, 2)
        if layer_mix != None:
            return x,target_a,target_b,target_reweighted,portion
        else:
            return x
