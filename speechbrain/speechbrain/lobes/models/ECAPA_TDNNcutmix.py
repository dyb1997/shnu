...
class ECAPA_TDNN2(torch.nn.Module):
    ...
    def cutmix(self, input, target, beta, use_cuda=True):
        ...
        # bbx1, bby1, bbx2, bby2 = self.rand_bbox(input.size(), lam)
        # 随机产生一个box的四个坐标
        input[:, :, bby1:bby2] = input[rand_index, :, bby1:bby2]
        lam = 1 - ((bby2 - bby1) / input.size()[-1])
        # 将样本i中box的像素值填充该批数据中相同区域
    ...
    def forward(self, x,spkid,stage, lengths=None):
        ...
        x = torch.cat(xl[1:], dim=1)
        r = np.random.rand(1)
        if r < 0.3 and stage==sb.Stage.TRAIN:
            x, y_a, y_b, lam = self.cutmix(x, spkid, 0.5)
        else:
            x = x
            y_a = y_b= spkid
            lam=1
