...
class Brain:
    ...
    * ``evaluate_batch()``
    ...
    def compute_forward(self, batch, epoch,stage):
    ...
    def fit_batch(self, batch,epoch):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            outputs = self.compute_forward(batch, epoch,Stage.TRAIN)
            loss = self.compute_objectives(outputs, batch, Stage.TRAIN)
            loss.backward()
            if self.check_gradients(loss):
                self.optimizer.step()
    ...
    def evaluate_batch(self, batch,epoch, stage):
        ...
        * ``compute_objectives()``
        ...
        out = self.compute_forward(batch,epoch, stage=stage)
        ...
        * ``update_average()``
        ...
        for epoch in epoch_counter:
            ...
            with tqdm(
                ...
                for batch in t:
                    self.step += 1
                    loss = self.fit_batch(batch,epoch)
