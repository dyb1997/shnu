...
class Brain:
    ...
    * ``evaluate_batch()``
    ...
    def __init__(  # noqa: C901
        ...
        checkpointer2=None,
        max_epoch = None,
        max_consistency_cost = None,
    ):
        self.max_epoch = max_epoch
        self.max_consistency_cost = max_consistency_cost
        self.opt_class = opt_class
        self.checkpointer = checkpointer
        self.opt_class2 = opt_class2
        self.checkpointer2 = checkpointer2
        ...
        self.modules2 = torch.nn.ModuleDict(modules2).to(self.device)
        ...
        if (
            ...
            logger.info(
                ...
                f"output to temporary directory: {tempdir.name + 'teacher'}"
            )
            self.checkpointer.checkpoints_dir = pathlib.Path(tempdir.name + "teacher")
        ...
        if (
                self.debug
                and self.checkpointer2 is not None
                and hasattr(self.checkpointer2, "checkpoints_dir")
        ):
            tempdir = tempfile.TemporaryDirectory()
            logger.info(
                "Since debug mode is active, switching checkpointer "
                f"output to temporary directory: {tempdir.name + 'student'}"
            )
            self.checkpointer2.checkpoints_dir = pathlib.Path(tempdir.name + "student")

            # Keep reference to tempdir as long as checkpointer exists
            self.checkpointer2.tempdir = tempdir
        ...
        if total_params > 0:
            ...
            logger.info(f"{fmt_num + 'teacher'} trainable parameters in {clsname}")

        total_params2 = sum(
            p.numel() for p in self.modules2.parameters() if p.requires_grad
        )
        if total_params2 > 0:
            clsname = self.__class__.__name__
            fmt_num = sb.utils.logger.format_order_of_magnitude(total_params)
            logger.info(f"{fmt_num + 'student'} trainable parameters in {clsname}")
        ...
        # Prepare iterating variables(teacher)
        self.avg_train_loss = 0.0
        self.step = 0
        # Prepare iterating variables(student)
        ...
        # Add this class to the checkpointer for intra-epoch checkpoints
        if self.checkpointer2 is not None:
            self.checkpointer2.add_recoverable("brain2", self)
    ...
    def make_dataloader(
        ...
        **loader_kwargs : dict
        ...
        if (
            ...
            ckpt_key = ckpt_prefix + stage.name + "teacher"
            self.checkpointer.add_recoverable(ckpt_key, dataloader)
        if (
            self.checkpointer2 is not None
            and ckpt_prefix is not None
            and (
                isinstance(dataloader, SaveableDataLoader)
                or isinstance(dataloader, LoopedLoader)
            ...
            ckpt_key = ckpt_prefix + stage.name +"student"
            self.checkpointer2.add_recoverable(ckpt_key, dataloader)
        return dataloader
    ...
    def exp_rampup(self,current, rampup_length):
    ...
    def on_fit_start(self):
        ...
        if self.checkpointer2 is not None:
            self.checkpointer2.recover_if_possible(
                device=torch.device(self.device)
            )
    ...
    def init_optimizers(self):
        ...
        if self.opt_class2 is not None:
            self.optimizer2 = self.opt_class2(self.modules2.parameters())

            if self.checkpointer2 is not None:
                self.checkpointer2.add_recoverable("optimizer2", self.optimizer2)
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            with torch.cuda.amp.autocast():
                outputs_T,outputs_S,len_T,len_S = self.compute_forward(batch, Stage.TRAIN)
                loss1 = self.compute_objectives(outputs_T, len_T, batch, Stage.TRAIN)
                loss2 = self.compute_objectives(outputs_S, len_S, batch, Stage.TRAIN)
                loss3 = self.MSE(outputs_T,outputs_S)
            ...
            outputs_T, outputs_S, outputs_C,len_T, len_S ,len_C,embedding_T,embedding_S,embedding_C,y,y_T= self.compute_forward(batch, sb.Stage.TRAIN)
            loss = None
            # print("emb",embedding_C.shape)
            # loss1 = self.compute_objectives(outputs_T, len_C, batch, sb.Stage.TRAIN)
            # # print("loss1",loss1)
            # meters.update('teacher-aamsoftmax', loss1.item())
            # if loss is not None:
            #     loss += loss1
            # else:
            #     loss = loss1
            # loss2 = self.compute_objectives2(outputs_S, len_C, batch, sb.Stage.TRAIN)
            # # loss2 = self.compute_objectives3(embedding_S,batch, sb.Stage.TRAIN)
            # # print("loss2",loss2)
            # meters.update('student-aamsoftmax', loss2.item())
            # if loss is not None:
            #     loss += loss2
            # else:
            #     loss = loss2
            loss2 = self.compute_objectives3(y_T, batch, sb.Stage.TRAIN)
            meters.update('SCL-loss', loss2.item())
            print("loss2", loss2)
            if loss is not None:
                loss += loss2
            else:
            ...
            loss3 = self.compute_objectives3(y,batch, sb.Stage.TRAIN)
            meters.update('SCL-loss', loss3.item())
            print("loss",loss3)
            if loss is not None:
                loss += loss3
            else:
            ...
            # meters.update('SCL-loss', loss3.item())
            # #loss3 = 0.1*loss3
            loss4 = self.MSE(embedding_T,embedding_S)
            loss4 = consistency_cost*loss4
           # meters.update('MSE-loss', loss4.item())
            if loss is not None:
                loss += loss4
            else:
                loss = loss4
            # print("loss4MSE",loss4.shape)
            loss.backward()
            if self.check_gradients(loss):
                self.optimizer.step()
    ...
    def check_gradients(self, loss):
        ...
        if not torch.isfinite(loss):
            ...
            for p in self.modules2.parameters():
                if not torch.isfinite(p).all():
                    logger.warn("Parameter2 is not finite: " + str(p))
        ...
        torch.nn.utils.clip_grad_norm_(
            (p for p in self.modules2.parameters()), self.max_grad_norm
        )
    ...
    def evaluate_batch(self, batch, stage):
        ...
        * ``compute_objectives()``
        ...
        detached loss
        """
        # outputs_T, outputs_S, len_T, len_S = self.compute_forward(batch, stage=stage)
        # loss1 = self.compute_objectives(outputs_T, len_T, batch, stage=stage)
        # loss2 = self.compute_objectives(outputs_S, len_S, batch, stage=stage)
        # loss3 = self.MSE(outputs_T, outputs_S)
        # loss = loss2 + loss3
        meters = AverageMeterSet()
        outputs_T, outputs_S, outputs_C,len_T, len_S ,len_C,embedding_T,embedding_S,embedding_C,y= self.compute_forward(batch, stage=stage)
        loss1 = self.compute_objectives(outputs_C, len_C, batch, stage=stage)
        loss2 = self.compute_objectives2(outputs_C, len_C, batch, stage=stage)
        # loss3 = self.MSE(embedding_T, embedding_S)
        # consistency_cost = self.max_consistency_cost * self.rampup_value
        # loss3=consistency_cost*loss3
        # loss4 = consistency_costself.MSE(y_T, y_S)
        # lossse1 = self.MSE(se1_T, se1_S)
        # lossse2 = self.MSE(se2_T, se2_S)
        # lossse3 = self.MSE(se3_T, se3_S)
        # lossse = (lossse1 + lossse2 + lossse3) * 0.5
        #loss3 = 0.1 * loss3
        loss=loss1+loss2
    ...
    def fit(
        ...
        * ``update_average()``
        ...
        self.on_fit_start()#初始化优化器，并加载最新的检查点以恢复训练。
        #是否在每个epoch中显示进度
        ...
        for epoch in epoch_counter:
            ...
            with tqdm(
                ...
                for batch in t:
                    self.step += 1
                    self.global_step = epoch * len(train_set) + self.step
                    self.rampup_value = self.exp_rampup(self.global_step, self.max_epoch * len(train_set))
                    #batch有两个元素：输入和目标。批次有两个元素：输入和目标。
    ...
    def _compile_jit(self):
        ...
        for name in self.jit_module_keys:
            ...
            if name not in self.modules2:
                raise ValueError(
                    "module2" + name + " is not defined in your hparams file."
                )
            module = torch.jit.script(self.modules[name])
            module2 = torch.jit.script(self.modules2[name])
            self.modules[name] = module.to(self.device)
            self.modules2[name] = module2.to(self.device)
    ...
    def _wrap_distributed(self):
        ...
        if not self.distributed_launch and not self.data_parallel_backend:
            ...
            for name, module2 in self.modules2.items():
                if any(p.requires_grad for p in module2.parameters()):
                    module2 = SyncBatchNorm.convert_sync_batchnorm(module2)
                    module2 = DDP(
                        module2,
                        device_ids=[self.device],
                        find_unused_parameters=self.find_unused_parameters,
                    )
                    self.modules2[name] = module2
        else:
            # data_parallel_backend
            for name, module2 in self.modules2.items():
                if any(p.requires_grad for p in module2.parameters()):
