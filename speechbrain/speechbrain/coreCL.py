    ...
    * ``evaluate_batch()``
    ...
    def fit_batch(self, batch):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            loss = self.compute_objectives3(y, batch,len, sb.Stage.TRAIN)
            loss.backward()
            if self.check_gradients(loss):
                self.optimizer.step()
    ...
    def evaluate_batch(self, batch, stage):
        ...
        * ``compute_objectives()``
        ...
        outputs,lens,emb ,y = self.compute_forward(batch, stage=stage)
        loss = self.compute_objectives(o, batch, stage=stage)
        return loss.detach().cpu()
