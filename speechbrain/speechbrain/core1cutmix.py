...
class Brain:
    ...
    * ``evaluate_batch()``
    ...
    def compute_forward(self, batch,wavs2,spkids, stage):
    ...
    def fit_batch(self, batch,wavs2,spkid):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            #torch.cuda.amp.autocast()通过放大loss的值来防止梯度的underflow（只在BP时传递梯度信息使用，真正更新权重时还是要把放大的梯度再unscale回去）
            with torch.cuda.amp.autocast():
                outputs = self.compute_forward(batch,wavs2,spkid, sb.Stage.TRAIN )
    ...
    def check_gradients(self, loss):
        ...
        # Clip gradient norm(nn.utils.clip_grad_norm(parameters, max_norm, norm_type=2)设置一个梯度剪切的阈值，如果在更新梯度的时候，梯度超过这个阈值，则会将其限制在这个范围之内，防止梯度爆炸。)
    ...
    def evaluate_batch(self, batch, wavs2,spkids, stage):
        ...
        * ``compute_objectives()``
        ...
        out = self.compute_forward(batch, wavs2,spkids, stage=stage)
    ...
    def fit(
        ...
        * ``update_average()``
        ...
        for epoch in epoch_counter:
            ...
            self.modules.train()
            movise_set = pd.read_csv(movise_csv)
            movisepath = movise_set["wav"]  # 获取一列，用一维数据
            movisestar = movise_set["start"]
            duration = movise_set["duration"]
            moviseid = movise_set["spk_id"]
            ...
            lab_enc_file = os.path.join(movise_csv.rsplit("/",1)[0],"label_encoder.txt")
            ...
            with tqdm(
                ...
                for batch in t:
                    ...
                    for i in range(len(batch)):
                        a = random.randint(0,len(movisepath)-1)
                        snt_len_sample = int(32000)
                        duration_sample = int(duration[a] * 16000)
                        start = random.randint(0, duration_sample - snt_len_sample - 1)
                        stop = start + snt_len_sample
                        num_frames = stop - start
                        sig, fs = torchaudio.load(
                            movisepath[a], num_frames=num_frames, frame_offset=start
                        )
                        sig = sig.transpose(0, 1).squeeze(1)
                        wavs2.append(sig)
                        lab2ind, ind2lab, extras=label_encoder._load_literal(lab_enc_file)

                        spk_id_encoded = lab2ind[moviseid[a]]
                        # print(spk_id_encoded)
                        spkid.append([spk_id_encoded])
                    wavs2=torch.tensor([item.cpu().detach().numpy() for item in wavs2]).cuda()
                    spkid=torch.tensor([spkid]).cuda().squeeze(0)
                    # print(spkid.shape)
                    # wavs2= torch.cat(wavs2.unsqueeze(), dim=-1).cuda()
                    # spkid = torch.cat(spkids, dim=0).cuda()
                    self.step += 1
                    loss = self.fit_batch(batch,wavs2,spkid)
