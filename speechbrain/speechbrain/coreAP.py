    ...
    def fit_batch(self, batch):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            outputs1,lens,outputs2,lens2,embeddings1,embeddings2,outputs1r,outputs2r= self.compute_forward(batch, sb.Stage.TRAIN)
            loss1 = self.compute_objectives(embeddings1,embeddings2,batch,sb.Stage.TRAIN)
            loss2 = self.ssreg(outputs1,outputs2,outputs1r.detach(),outputs2r.detach())
            loss = loss1+loss2
            # embeddings1,embeddings2= self.compute_forward(batch, sb.Stage.TRAIN)
            # loss = self.compute_objectives(embeddings1,embeddings2,batch,sb.Stage.TRAIN)
            loss.backward()
            if self.check_gradients(loss):
                self.optimizer.step()
            self.optimizer.zero_grad()
