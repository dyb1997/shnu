    ...
    * ``evaluate_batch()``
    ...
    def _train_loader_specifics(self, dataset, loader_kwargs):
        ...
        if self.distributed_launch and not isinstance(dataset, IterableDataset):
            ...
            # num_replicas arg is equal to world_size.2
    ...
    def fit_batch(self, batch):
        ...
        * ``compute_objectives()``
        ...
        if self.auto_mix_prec:
            ...
            outputs_T, outputs_S, len_T, len_S ,embedding_T,embedding_S,y_T,y_S,A_T,A_S= self.compute_forward(batch, sb.Stage.TRAIN)
            loss = None
            ...
            #     loss = loss3
            ##################################
            loss3 = self.MSE(A_T,A_S)
            loss3 = (consistency_cost+1)*loss3
            print("loss3",loss3)
            meters.update('MSE-loss', loss3.item())
            #loss3 = 0.1*loss3
            if loss is not None:
                ...
                loss = loss3
            ##################################
            # loss4 = self.MSE(y_T, y_S)
            # loss4 = consistency_cost * loss4
            # # loss3 = self.MSE(outputs_T, outputs_S)
            # print("loss4",loss4)
            # meters.update('MSE-loss1', loss4.item())
            # # loss3 = 0.1*loss3
            # if loss is not None:
            #     loss += loss4
            # else:
            ...
            # else:
            #     loss = lossse
            # meters.update('Lossse1', lossse1.item())
            # meters.update('Lossse2', lossse2.item())
            # meters.update('Lossse3', lossse3.item())
            # meters.update('Lossse', lossse.item())
            # loss = loss1+ loss2 + loss3
            loss.backward()
            if self.check_gradients(loss):
                self.optimizer.step()
    ...
    def evaluate_batch(self, batch, stage):
        ...
        * ``compute_objectives()``
        ...
        # loss = loss2 + loss3
        meters = AverageMeterSet()
        outputs_T, outputs_S, len_T, len_S ,embedding_T,embedding_S,y_T,y_S= self.compute_forward(batch, stage=stage)
        loss1 = self.compute_objectives(outputs_T, len_T, batch, stage=stage)
