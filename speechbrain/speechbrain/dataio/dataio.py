...
def length_to_mask(length, max_len=None, dtype=None, device=None):
    ...
    length : torch.LongTensor
        Containing the length of each sequence in the batch. Must be 1D.包含批次中每个序列的长度。 必须是一维的
    max_len : int
        Max length for the mask, also the size of the second dimension.掩码的最大长度，也是第二维的大小。
