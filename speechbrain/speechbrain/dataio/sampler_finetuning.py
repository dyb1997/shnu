...
class voxmoviesFTBatchSampler(Sampler):
    '''
    ...
    def __init__(self, sampler):
        self.spk = 16
        self.num = 2
        self.data = sampler.data_source.data
        # self.data_set = self.data.data_set
        # self.data_id = data.spk_id
        self.batch_size = self.num * self.spk
        self.drop_last = True
        self.voxmovies = []
        self.voxmovies_spk = []
        self.vox2 = []
        for k,v in self.data.items():
            if v.get("data_set") == "voxmovies_trains":
                self.voxmovies.append(k)
        self.voxmovies_len = len(self.voxmovies)
    ...
    def __iter__(self):
        ...
        while len(voxmovies) > self.spk :
            ...
            while i < self.spk:
                n=1
                i=i+1
                movies = random.choice(voxmovies)
                batch.append(movies)
                voxmovies.remove(movies)
                while n<self.num:
                    for j,l in self.data.items():
                        if l.get("data_set") == "voxceleb2" and l.get("spk_id")== self.data[movies].get("spk_id") and n<self.num:
                            batch.append(j)
                            n = n+1
            if len(batch) == self.batch_size and m <= (self.voxmovies_len//self.spk):
                yield batch
            else:
                break
    ...
    # def __next__(self):
    #     if self.m >= (self.voxmovies_len//self.spk):
    #         self.m=self.m+1
    #         return self.next()
    #     else:
    #         exit()
    ...
    def __len__(self):
        if self.drop_last:
            return self.voxmovies_len//self.spk
        else:
            return (self.voxmovies_len +self.spk -1) // self.spk
        # if self.drop_last:
        #     return self.len // self.spk
        # else:
        #     return (self.len + self.spk - 1) // self.spk
